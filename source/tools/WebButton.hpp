...
private:
    ...
    std::string label           = DEFAULT_LABEL;               ///< Button text content
    bool        is_enabled      = true;                        ///< Whether button accepts clicks
    bool        is_pressed      = false;                       ///< Toggle state
    std::function<void()> on_click_callback;                   ///< Function to call on click

    int         width         = DEFAULT_WIDTH;                 ///< Button width (px)
    int         height        = DEFAULT_HEIGHT;                ///< Button height (px)
    std::string bg_color      = DEFAULT_BG_COLOR;              ///< Background color
    std::string text_color    = DEFAULT_TEXT_COLOR;            ///< Text color
    std::string border_color  = DEFAULT_BORDER_COLOR;          ///< Border color
    int         border_width  = DEFAULT_BORDER_WIDTH;          ///< Border width (px)
    int         border_radius = DEFAULT_BORDER_RADIUS;         ///< Border radius for rounded corners

    static inline int next_id = 0;                             ///< Counter for generating unique IDs

#ifdef __EMSCRIPTEN__
    emscripten::val dom_element;

    // Global registry mapping button IDs to instances (for JS callbacks)
    static inline std::unordered_map<std::string, WebButton*> button_registry;
...
public:
     ...
     */
    [[nodiscard]] const std::string& GetElementID() const;
