...
namespace cse498
{
    ...
    double WorldPath::CrossProd(const Point& o, const Point& a, const Point& b)
    {
    ...
    {
        ///checks collienearity
        const double xProd = CrossProd(p, a, b);

        if (SignWithEps(xProd) != 0) return false;
    ...
    {
        const double ab_c = CrossProd(a, b, c);
        const double ab_d = CrossProd(a, b, d);
        const double cd_a = CrossProd(c, d, a);
        const double cd_b = CrossProd(c, d, b);
    ...
    ///Checks if there is any self intersection throughout the whole path
