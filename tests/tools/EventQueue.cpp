...
TEST_CASE("Test EventQueue Comparisons", "[EventQueue]") 
{
    ...
    REQUIRE(eq2.Top().GetData() == "Event C");
    eq2.Pop();
    REQUIRE(eq2.Top().GetData() == "Event B");
    eq2.Pop();
    REQUIRE(eq2.Top().GetData() == "Event A");
}

TEST_CASE("Test EventQueue Interleaved Priorities", "[EventQueue]")
{
    // Test that events with interleaved priorities are ordered correctly
    EventQueue eq;
...
TEST_CASE("Test Clear", "[EventQueue]")
{
    // Test the Clear function of the EventQueue
    EventQueue eq;
    ...
    eq.Push(Event("Event 4", 1));
    REQUIRE(!eq.Empty());
    REQUIRE(eq.Size() == 1);
    REQUIRE(eq.Top().GetData() == "Event 4");
}