 ...
 * @author Group 06
 * @brief Integration tests for ClassicAgent using the full Sense → tree → GetAction pipeline.
 *
 * Unlike DynamicTreeBuilderTests, these tests go through ClassicAgent::SelectAction(),
 * which calls Sense() to populate the blackboard and then runs the tree. This verifies
 * that the complete decision pipeline produces correct action IDs against a real world.
 *
 * NOTE: ClassicAgent::Sense() currently sets wood/stone/steel/wheat counts and
 * explore_move, but does NOT set on_grass. Building branches in DynamicTreeBuilder
 * require on_grass=true to fire, so they will not activate through this pipeline until
 * Sense() is updated. The tests below cover what is currently working: movement/exploration.
 */
...
// Named CATestWorld to avoid collision with TestDynamicWorld in DynamicWorldTests.cpp.
...
TEST_CASE("ClassicAgent - initializes successfully in DynamicWorld", "[ClassicAgent][init]") {
    CATestWorld world;
...
TEST_CASE("ClassicAgent - returns a valid movement action when exploring", "[ClassicAgent][move]") {
    CATestWorld world;
    // Place agent in open space with no resources — tree will use explore fallback.
    auto& agent = world.SpawnClassicAt(50, 50);
    ...
    bool is_valid_action = action == CATestWorld::MOVE_UP    ||
                           ...
                           action == CATestWorld::MOVE_RIGHT ||
                           action == 0;
    REQUIRE(is_valid_action);
}

TEST_CASE("ClassicAgent - does not return a build action when resources are zero", "[ClassicAgent][move]") {
    CATestWorld world;
    auto& agent = world.SpawnClassicAt(50, 50);
    world.SetCell(50, 50, world.GrassId());
...
TEST_CASE("ClassicAgent - SelectAction does not crash when agent is at grid boundary", "[ClassicAgent][move]") {
    CATestWorld world;
...
TEST_CASE("ClassicAgent - SelectAction returns consistent type across multiple calls", "[ClassicAgent][move]") {
    CATestWorld world;
    auto& agent = world.SpawnClassicAt(50, 50);
    world.SetCell(50, 50, world.GrassId());

    // Call SelectAction multiple times; should never throw and always return size_t.
    for (int i = 0; i < 5; ++i) {
        REQUIRE_NOTHROW(agent.SelectAction(world.GetGrid()));
    }
}
...
TEST_CASE("ClassicAgent - Sense picks up wood count from world", "[ClassicAgent][sense]") {
    CATestWorld world;
    auto& agent = world.SpawnClassicAt(50, 50);
    world.SetCell(50, 50, world.GrassId());
...
TEST_CASE("ClassicAgent - moves toward nearest visible resource tile", "[ClassicAgent][sense]") {
    CATestWorld world;
    auto& agent = world.SpawnClassicAt(50, 50);
    world.SetCell(50, 50, world.GrassId());
