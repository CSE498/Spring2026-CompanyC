...
### 2. How Agents Interact with the World
...
### 3. How Agents Interact with Each Other
...
I believe efficient state management will be important as the grid could be a bottleneck in the program if not fast enough. The entire program will run based off the StateGrid so we need to make sure optimizations are made for performance.
...
### 2. Similar Standard Library Classes to Learn From
...
**(2) Recoverable errors (exception)**
...
**(3) User/input errors (special return / optional)**
...
### 5. Expected Challenges / Topics to Learn

- **Designing a clean interface boundary** between StateGridPosition and StateGrid: Should StateGridPosition be pure data (row/col/facing only), or should it also provide grid-aware queries via passing a `StateGrid&`?
- **Direction/orientation design**: Picking a simple `enum class Direction { North, East, South, West };`, handling rotation and forward movement consistently.
- **Avoiding "silent bugs"**: Clear semantics for row/col vs x/y, and consistent coordinate system.

### 6. Other Groups / Classes to Coordinate with
...
**Across the company, likely coordination points:**
...
### Class Scheduler (Deterministic Stride Version)
...
- **Pass Value Overflow**: If the simulation runs for a very long time, the Pass values (Virtual Time) will eventually overflow. Research Virtual Time Wraparound techniques or periodic "normalization" (subtracting the minimum Pass from all processes) to keep the numbers manageable.

- **Pointer Invalidation**: If using a `std::priority_queue`, updating an existing element's priority is difficult because the STL doesn't support "re-heaping" an arbitrary element. May need to implement a custom heap or use a `std::set` of structs to allow for efficient updates.
...
### 1. Overall Vision / Class Description
...
- `Evaluate(compiledExpr, variables)`: Evaluates the compiled expression using runtime variable values. Supported features include numeric constants, variables, + - * / operators, parentheses, and unary minus
...
### 6. Other Classes to Coordinate with
...
- `std::unordered_map<std::string, T>`: For the basic key-value association and O(1) average time complexity lookups.
- `std::any`: For storing values of any type in a single container.
- `std::variant`: For type-safe unions.
- `std::type_index / std::type_info`: For identifying and comparing the types of stored objects at runtime.
