...
Hi, I’m Sadwal Patel from Group 24, and in this video I’m going to go over my main contributions to the project, both on the class side and on the overall module side.

For Company C, Group 24 was responsible for the web interface module, so our job was not just to build a few frontend-looking classes, but to build a web interface that could actually plug into the larger agent-based system and work with different world modules through the project API. That’s really the lens I approached my work through.

The class I took the lead on was WebImage. This was one of the assigned Group 24 classes, and its purpose was to let C++ code manage HTML images in a clean way instead of forcing the rest of the module to directly deal with DOM image logic every time. The project description for Group 24 specifically describes WebImage as the class that manages an HTML image from C++ and handles things like asset loading, size, position, and related display behavior.
What I did here was build WebImage as an actual reusable wrapper, not just a demo object. So in the header and implementation, I set it up to manage source, alt text, geometry, visibility, opacity, z-index, parent attachment, element id, CSS classes, and inline styles. I also built the JavaScript bridge through Emscripten so the class could actually create and update real browser image elements from C++, and I included native no-op stubs so the same code could still be compiled and tested outside the browser environment. You can see that directly in the implementation here, where there’s both the Emscripten bridge layer and the native fallback path.

For me, that class mattered because it became part of the web-side foundation. It was not just one isolated file. It was one of the building blocks that let the rest of the interface use images in a consistent and reusable way.
Beyond that class, a big part of my contribution was on the module and integration side. I created and worked on interface-side support files like IWorldUiAdapter.hpp and WebAssetKeys.hpp, and more broadly I spent a lot of time on the adapter-style and integration logic that connected the web layer to different worlds. That mattered because our module could not assume only one fixed world representation. Group 24 had to work with multiple world teams, especially the dynamic and interaction-heavy world modules, so the interface needed to be flexible and reusable instead of hardcoded around one setup. Company C was explicitly structured as a multi-group system where modules needed to plug together, so that interoperability side was a big part of the real work.

On the module side, I worked on the parts that made the interface more usable and more connected to the other groups’ modules. That included world-driven panel behavior, action and selection flow, and general world hookup logic through the interface layer. In other words, I was doing more than class implementation. I was helping build the connection between reusable web components and the actual game or simulation worlds they needed to support.

So the way I’d summarize my contribution is this: I took the lead on WebImage, but my work also extended into a lot of the integration and support structure that helped turn Group 24’s code into a working module instead of just a set of separate web utilities.
